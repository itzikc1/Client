...
public class SelectGameWindow extends BasicWindow {
	...
	public SelectGameWindow(Presenter presenter, int width, int height,
			String title) {
	...
	void initWidgets() {
		...
		btnSelectModel.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false,
				false, 2, 1));
		btnSelectModel.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				if (message.getSetselected().equals("Maze")) {
					PropretiesWindow mazepro = new PropretiesMaze(presenter,
							display, 400, 300, "Maze Propreties");
					mazepro.run();
					// BasicWindow window = new MazeGameWindow(400, 300,
					// "Maze Game");
					// window.run();
				}
				if (message.getSetselected().equals("EightPuzzle")) {
					PropretiesWindow puzzle = new EightPuzzleProprties(presenter,display, 500, 500, "Eigth Puzzlee Propreties");
					puzzle.run();
				}
			}
